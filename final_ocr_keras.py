        ...
        for tr in tracked:
            ...
            if hasattr(p_res, 'boxes') and p_res.boxes is not None:
                for pb, pconf in zip(p_res.boxes.xyxy, p_res.boxes.conf):
                    ...
                    ocr_res = pipeline.recognize([plate_crop])[0]  # Returns list of (text, box) tuples
                    print(f"Frame {frame_idx}, ID {tid}: Raw OCR results: {ocr_res}")

                    if ocr_res:
                        # Combine all detected words into a single string (license plates are typically single words)
                        text = ''.join([word for word, _ in ocr_res])
                        ...
                        if cleaned:
                            ...
                            break  # Take the first valid plate detection
